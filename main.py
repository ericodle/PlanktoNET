...
def main(data_dir, output_dir, learning_rate=0.0001, num_imgs=100):
    ...
    for class_idx, class_name in enumerate(class_names):
        ...
        # Log the class name and number of available images
        print(f"Class: {class_name}, Available images for training: {len(filtered_image_paths)}")
    ...
    resnet_model = timm.create_model('resnet50', pretrained=True, drop_rate=0.2, num_classes=len(class_names))
    ...
    optimizer = torch.optim.Adam(resnet_model.parameters(), lr=learning_rate, weight_decay=1e-4)
    patience = 5
