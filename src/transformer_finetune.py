...
def main(data_dir, model_path, output_dir, learning_rate, num_imgs):
    ...
    # Load pre-trained Vision Transformer model from timm
    vit_model = timm.create_model('vit_base_patch16_224', pretrained=True, num_classes=len(dataset.classes))
...
if __name__ == "__main__":
    if len(sys.argv) != 6:
        print("Usage: python script.py <train/test image directory> <model path> <output directory> <initial learning rate> <images per class>")
        sys.exit(1)
    data_dir = sys.argv[1]
    ...
    main(data_dir, model_path, output_dir, learning_rate, num_imgs)
