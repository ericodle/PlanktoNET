...
def main(data_dir, output_dir, learning_rate):
    ...
    for class_idx, class_name in enumerate(testid_classes):
        ...
        if len(image_paths) > 200:
            image_paths = random.sample(image_paths, 200)  # Randomly select 100 images
        
        # Apply data augmentation if class has less than 200 images
        if len(image_paths) < 200:
    ...
    resnet101.fc = torch.nn.Linear(num_features, len(testid_classes))
    ...
    optimizer = torch.optim.Adam(resnet101.parameters(), lr=learning_rate) 
    ...
    with open(log_file_path, 'w') as log_file:
        ...
        for epoch in range(num_epochs):
            ...
            if epochs_since_improvement >= 5:
                print("Early stopping triggered. Test accuracy did not improve within 5 epochs.")
                stop_training = True
                break
