...
def main(data_dir, output_dir, learning_rate):
    ...
    for class_idx, class_name in enumerate(testid_classes):
        ...
        if len(image_paths) > 100:
            image_paths = random.sample(image_paths, 100)  # Randomly select 30 images
        
        # Apply data augmentation if class has less than 100 images
        if len(image_paths) < 100:
            ...
            num_additional_samples = 100 - len(image_paths)
    ...
    vgg16.classifier[6] = torch.nn.Linear(num_features, len(testid_classes))
