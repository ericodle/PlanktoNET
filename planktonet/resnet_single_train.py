...
'''
This script utilizes a pretrained ResNet101 model to train and evaluate a custom image dataset, handling data preprocessing, model training with early stopping based on test accuracy, and generating visualizations of training metrics. It saves the trained model, logs, and plots to the specified output directory, enabling analysis and reusability.
'''
...
def main(data_dir, output_dir, learning_rate, num_imgs):
    ...
    resnet101_model = torchvision.models.resnet101(weights=ResNet101_Weights.DEFAULT)
    num_ftrs = resnet101_model.fc.in_features
    resnet101_model.fc = torch.nn.Linear(num_ftrs, len(class_names))  # Change the last fully connected layer to match the number of classes
